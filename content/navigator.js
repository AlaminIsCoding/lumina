...
const Navigator = {
    ...
    createUI: () => {
        ...
        // 1. The List (Hidden by default)
        const list = document.createElement('div');
        list.id = 'lumina-navigator-list';
        container.appendChild(list);

        // 2. The Pill (Always visible if notes exist)
    ...
    render: () => {
        ...
        // Update Count
        document.getElementById('lumina-nav-count').textContent = Navigator.highlights.length;
        ...
        sorted.forEach(h => {
            ...
            const icon = h.note ? '📝 ' : '';
            item.textContent = icon + (h.text.substring(0, 30) + (h.text.length > 30 ? '...' : ''));
